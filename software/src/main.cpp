...
int main(int argc, char **argv) {
  ...
  std::cout << "Iniciando Artix7-Futures-Accelerator Host Engine..."
            << std::endl;
  ...
  if (!uart.openPort(uartPort)) {
    std::cout << "[!] Ejecutando en modo SIMULACIÓN (sin FPGA conectada)."
              << std::endl;
  }
  ...
  if (client.connect("127.0.0.1", 4002, 1)) {
    ...
    // (1) para live
    client.client()->reqMarketDataType(1);
    ...
    // contract.lastTradeDateOrContractMonth = "202609"; // Ajustar al
    // vencimiento activo

    // std::cout << "[+] Suscribiendo a flujo de mercado (MES)..." << std::endl;
    // client.client()->reqMktData(1001, contract, "", false, false,
    // TagValueListSPtr());

    // Definición del Contrato Cripto 24/7
    Contract contract;
    contract.symbol = "BTC";
    ...
    std::cout << "[+] Suscribiendo a flujo 24/7 (BTC/USD)..." << std::endl;
    client.client()->reqMktData(1001, contract, "", false, false,
                                TagValueListSPtr());
    ...
    std::cerr << "[-] Error al conectar con IB Gateway en 127.0.0.1:4002."
              << std::endl;
    return 1;
  }
